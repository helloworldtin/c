...
int main(void)
{
    ...
    for(;;){
        ...
        switch (code) {
            ...
            case 'p':   print();
                        break;
            case 'd':   save_in_database();
                        break;
            case 'r':   load_from_database();
                        break;
            case 'q':   return 0;
...
/**********************************************************************
 * save_in_database:   Save the current inventory in file provided    *
                       by the user.                                   *
**********************************************************************/

void save_in_database()
{
    char filename[100]; // assume file name is no longer than 99 char
    FILE *fp;
    int n;
    ...
    n = fwrite(inventory, sizeof(struct part), num_parts, fp);
    if(n != num_parts) {
        fprintf(stderr, "Unable to write data in database\n");
        exit(EXIT_FAILURE);
    }

    printf("Data Successfully wrote in the database");
    fclose(fp);
    
}

/**************************************************************************
 * load_from_database:   Get  the current inventory data from  provided   *
                         filename from user.                              *
***************************************************************************/
...
void load_from_database()
{
    ...
    n = fread(temp_inventory, sizeof(struct part), num_parts, fp);
    if(n != num_parts) {
        fprintf(stderr, "Unable to read inventory data\n");
        exit(EXIT_FAILURE);
    }
    ...
    for(i = 0; i < num_parts; i++){
        printf("%7d         %-25s%11d\n", temp_inventory[i].number,
            temp_inventory[i].name, temp_inventory[i].on_hand);
    }
